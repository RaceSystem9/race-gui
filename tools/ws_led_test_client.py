...
def _print_result(parsed: Dict[str, Any], expected_color: str, elapsed_ms: int) -> bool:
    ...
    ok = message_type == "ack" and status.lower() == "ok" and server_color == expected_color.upper()
...
async def _run_client(host: str, port: int, path: str, color: str, timeout: float) -> int:
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            is_ok = _print_result(parsed, expected_color=color, elapsed_ms=elapsed_ms)
            return 0 if is_ok else 3
...
def main() -> None:
    ...
    exit_code = asyncio.run(_run_client(args.host, args.port, args.path, args.color, args.timeout))
